...
(function () {
  ...
  function updateDust(d, dt, i) {
    ...
    if (d.life >= d.maxLife || d.y < -20) { dust[i] = spawnDust(false); return; }
    const o = dustOpacity(d) * 0.35; // ฝุ่นให้จางๆ ไม่แย่งซีนฟ้าผ่า
    if (o <= 0.01) return;
    ctx.beginPath();
  ...
  // ================================================================
  const bolts = [];       // เส้นฟ้าผ่าหลัก + กิ่งที่กำลังแสดงผลอยู่
  let flashIntensity = 0; // แสงวาบทั่วจอตอนฟ้าผ่า (0..1)
  let nextStrikeAt = rand(1200, 2600); // เวลาของฟ้าผ่าครั้งแรก (ms นับจากเริ่ม)
  let clock = 0;

  // สร้างเส้นฟ้าผ่าแบบแตกกิ่งด้วยวิธี midpoint-displacement
  function buildBoltSegments(x1, y1, x2, y2, disp, segments, depth, branchChance) {
    ...
    // บางจุด แตกกิ่งเล็กๆ ออกด้านข้าง
  ...
  function spawnBolt() {
    ...
    const endY = height * rand(0.55, 0.92);
    const segments = [];
    buildBoltSegments(startX, 0, endX, endY, width * 0.09, segments, 0, 0.55);

    bolts.push({
      segments,
      life: 0,
      flashDur: rand(60, 110),   // วาบสว่างขึ้นเร็วมาก
      holdDur: rand(50, 120),    // ค้างสว่างสั้นๆ
      fadeDur: rand(500, 850),   // แล้วค่อยจางหาย
    });

    // บางครั้งผ่าซ้อน 2 เส้นให้ดูดุดันขึ้น
    if (Math.random() < 0.25) {
      const startX2 = rand(width * 0.1, width * 0.9);
      const endX2 = startX2 + rand(-width * 0.15, width * 0.15);
      const endY2 = height * rand(0.45, 0.85);
      const segments2 = [];
      buildBoltSegments(startX2, 0, endX2, endY2, width * 0.07, segments2, 0, 0.45);
      bolts.push({
        segments: segments2,
        life: 0,
  ...
  function drawBolt(b) {
    ...
    // ชั้นเรืองแสงหนา (glow) สีทองอุ่น
    ctx.beginPath();
    ...
    ctx.strokeStyle = `rgba(255,250,225,${o.toFixed(3)})`;
    ctx.lineWidth = 1.8;
    ctx.shadowBlur = 10;
  ...
  function drawScreenFlash() {
    ...
    const g = ctx.createRadialGradient(
      ...
      width * 0.5, height * 0.25, Math.max(width, height) * 0.9
    );
    g.addColorStop(0, `rgba(255,205,110,${(0.16 * flashIntensity).toFixed(3)})`);
    g.addColorStop(1, 'rgba(255,205,110,0)');
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, width, height);
    flashIntensity = Math.max(0, flashIntensity - 0.05); // แสงวาบทั่วจอหายไวกว่าตัวเส้น
  }
  ...
  function drawFrame(time) {
    ...
    if (!prefersReducedMotion) {
      ...
      if (clock >= nextStrikeAt) {
        spawnBolt();
        clock = 0;
        nextStrikeAt = rand(2500, 3600); // ทุกๆ ~3 วิ (สุ่มเล็กน้อยให้เป็นธรรมชาติ)
      }
      drawScreenFlash();
      updateBolts(dt);
      ...
      // โหมดลดการเคลื่อนไหว: วาดฟ้าผ่านิ่งเฟรมเดียวไว้เป็นของตกแต่ง
      spawnBolt();
      updateBolts(0);
