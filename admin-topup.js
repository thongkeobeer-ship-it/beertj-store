  ...
  <div class="admin-topup-card" data-id="${r.id}" data-user="${r.user_id || ''}" data-amount="${r.amount || 0}">
    ...
    <div class="admin-topup-body">
      ...
      <div class="admin-topup-actions">
        <button class="admin-btn admin-btn-ai" data-action="ai-verify">ກວດສອບດ້ວຍ AI</button>
...
async function runAiVerify(card, id, amount, btn){
  const box = document.getElementById(`aiBox-${id}`);
  const slipEl = card.querySelector('.admin-topup-slip');
  const imageUrl = slipEl ? slipEl.dataset.img : '';
  ...
  try {
    ...
    box.innerHTML = `
      <div class="ai-result ${cls}">
        ...
        <div>ຍອດທີ່ AI ອ່ານໄດ້: ${data.amount != null ? formatKip(data.amount) : '—'} (${data.matches_expected ? 'ກົງກັບຍອດ' : 'ບໍ່ກົງກັບຍອດ'})</div>
  ...
  document.querySelectorAll('.admin-topup-card').forEach(card => {
    ...
    card.querySelectorAll('.admin-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (action === 'ai-verify') {
          await runAiVerify(card, id, amount, btn);
          return;
        }
