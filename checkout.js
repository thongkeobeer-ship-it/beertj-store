...
document.addEventListener('DOMContentLoaded', () => {
  ...
  confirmBtn.addEventListener('click', async () => {
    ...
    // ອັບເດດຍອດເງິນທີ່ສະແດງຢູ່ໜ້າ (wallet chip) ໃຫ້ກົງກັບຍອດໃໝ່ທັນທີ ບໍ່ຕ້ອງລໍ reload
    const walletBalanceText = document.getElementById('walletBalanceText');
    if (walletBalanceText && typeof formatKipWallet === 'function') {
      walletBalanceText.textContent = formatKipWallet(data.new_balance);
    }

    document.getElementById('orderRef').textContent = 'ລະຫັດອ້າງອີງ: ' + data.ref;
