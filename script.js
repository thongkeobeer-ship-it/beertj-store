...
// === reveal ທົ່ວໄປ: ໃຫ້ card (ສິນຄ້າ ຫຼື ໝວດໝູ່) ເລື່ອນເຂົ້າມາຕິດກັບຕຳແໜ່ງ scroll ໂດຍກົງ (scroll-linked) ===
// ໃບທີ 1 ເລື່ອນເຂົ້າມາຈາກຊ້າຍ, ໃບທີ 2 ຈາກຂວາ, ສະຫຼັບກັນໄປເລື່ອຍໆ (ຊ້າຍ→ຂວາ→ຊ້າຍ→ຂວາ) — ບໍ່ມີ blur
...
// ບໍ່ມີ animation ໃດແລ່ນຕໍ່ໄປເອງຫຼັງຈາກຢຸດເລື່ອນ
...
const REVEAL_MAX_SHIFT = 70; // px ທີ່ເລື່ອນຈາກຊ້າຍ/ຂວາ
...
function updateRevealCard(card){
  ...
  const dir = card.dataset.revealDir === 'left' ? -1 : 1;
  const shiftX = dir * REVEAL_MAX_SHIFT * (1 - progress);
  const shiftY = 16 * (1 - progress);
  const scale = 0.97 + progress * 0.03;

  card.style.opacity = String(progress);
  card.style.transform = `translateX(${shiftX.toFixed(2)}px) translateY(${shiftY.toFixed(2)}px) scale(${scale.toFixed(3)})`;
}
...
function revealCardsOnScroll(cards){
  ...
  cardList.forEach((card, i) => {
    card.style.transition = 'none'; // ບໍ່ໃສ່ transition ໃດໆ -> ຕິດກັບ scroll ຈິງ 1:1 ທັນທີ
    if (!card.dataset.revealDir) {
      card.dataset.revealDir = (i % 2 === 0) ? 'left' : 'right';
    }
  });
