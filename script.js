...
// ໃຊ້ສະເພາະຕອນປຽບທຽບໝວດໝູ່ (ບໍ່ໄດ້ແກ້ຂໍ້ມູນຈິງ) — ຕັດຊ່ອງຫວ່າງຫົວ/ທ້າຍ ແລະ
// ຫຍໍ້ຊ່ອງຫວ່າງຊ້ຳກັນໃຫ້ເປັນຊ່ອງດຽວ ເພື່ອບໍ່ໃຫ້ພັງຖ້າພິມຊ່ອງຫວ່າງເກີນມາ
...
function productCardHtml(p){
  ...
  // ສິນຄ້າທີ່ມີ "ຕົວເລືອກໄລຍະເວລາ" ຈະບໍ່ມີລາຄາດຽວ (ແຕ່ລະໄລຍະເວລາລາຄາຕ່າງກັນ)
  // -> ສະແດງ "ເລີ່ມຕົ້ນ X" ຈາກລາຄາຕ່ຳສຸດແທນ
  let priceHtml;
  if (p.duration_enabled) {
    const prices = (p.durations || []).map(d => d.price).filter(v => v != null);
    priceHtml = prices.length
      ? `ເລີ່ມຕົ້ນ ${formatKip(Math.min(...prices))}`
      : 'ຫຼາຍໄລຍະເວລາ';
  } else {
    priceHtml = formatKip(p.price);
  }

  // ສະຖານະສິນຄ້າ: ມີລະຫັດເຫຼືອ -> ສີຂຽວ "ພ້ອມຂາຍ" / ລະຫັດໝົດ -> ສີແດງ "ສິນຄ້າໝົດ"
  const statusClass = outOfStock ? 'is-out' : 'is-live';
  const statusText = outOfStock ? 'ສິນຄ້າໝົດ' : 'ພ້ອມຂາຍ';
  ...
  <div class="product-card" data-id="${p.id}">
    ...
    <div class="product-body">
      ...
      <div class="product-footer">
        <div class="price shine-text">${priceHtml}</div>
...
async function loadStoreProducts(){
  ...
  if (!products || !products.length) {
    ...
    const withStock = await Promise.all(products.map(async (p) => {
      ...
      let durations = [];
      if (p.duration_enabled && typeof supabaseClient !== 'undefined') {
        const { data: durs } = await supabaseClient
      ...
      return { ...p, stock: stock ?? 0, durations };
    }));
...
function renderProducts(category){
  ...
  const filtered = (category && category !== 'all')
    ? allProducts.filter(p => normalizeCategory(p.category) === normCategory)
    : allProducts;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ຄຳຖາມທີ່ພົບເລື້ອຍ (FAQ) — ກົດເພື່ອເປີດ/ປິດຄຳຕອບ
