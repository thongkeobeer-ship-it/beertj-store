...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ปุ่มสั่งซื้อ -> ต้องล็อกอินก่อนถึงจะไปหน้ายืนยันคำสั่งซื้อได้
  const buyButtons = document.querySelectorAll('.buy-btn');
  buyButtons.forEach(btn => {
    btn.addEventListener('click', async () => {
      // เช็คสถานะล็อกอินก่อนทุกครั้ง (getCurrentUser มาจาก auth.js)
      ...
      const name = card?.querySelector('.product-title')?.textContent?.trim() || 'สินค้า';
      const category = card?.querySelector('.product-eyebrow')?.textContent?.trim() || '';
      ...
      const stock = stockMatch ? parseInt(stockMatch[1], 10) : 10;

      try {
        sessionStorage.setItem('selectedProduct', JSON.stringify({ name, category, price, stock }));
      } catch (e) { /* ถ้าเบราว์เซอร์บล็อก storage ก็ยังไปหน้าถัดไปได้ตามปกติ */ }
